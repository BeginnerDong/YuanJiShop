...
<script>
	export default {
		...
		onLoad(options) {
			const self = this;
			if (uni.getStorageSync('shopToken')) {
				uni.redirectTo({
					url: '/pages/merchantsentrance/merchantsentrance'
				})
			};
		},
				...
				if (self.$Utils.checkComplete(self.submitData)) {
					...
					const callback = (res) => {
						if (res.solely_code == 100000) {
							console.log(res);
							uni.setStorageSync('shopToken', res.token);
							uni.setStorageSync('shopNo', res.info.user_no);
