...
<script>
	export default {
		...
		onLoad(options) {
			const self = this;
			if (uni.getStorageSync('agentToken')) {
				uni.redirectTo({
					url: '/pages/agentportal/agentportal'
				})
			};
		},
				...
				if (self.$Utils.checkComplete(self.submitData)) {
					...
					const callback = (res) => {
						if (res.solely_code == 100000) {
							console.log(res);
							uni.setStorageSync('agentToken', res.token);
							uni.setStorageSync('agentNo', res.info.user_no);
							uni.setStorageSync('agentInfo', res.info);
							uni.redirectTo({
								url: '/pages/agentportal/agentportal'
							})
						} else {
